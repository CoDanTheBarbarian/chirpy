...
const userRedUpgrade = `-- name: UserRedUpgrade :exec
UPDATE users SET is_chirpy_red = true, updated_at = NOW()
WHERE id = $1
`

func (q *Queries) UserRedUpgrade(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, userRedUpgrade, id)
	return err
}
